...
- The attacker can no longer issue valid transactions from the compromised EOA (enforced by [EIP-3607]), and
...
| Constant | Value |
| - | - |
| `BALANCER_HARDFORK_TIMESTAMP` | _TBD_ |
...
Thanks to [EIP-3607], even with the attacker controlling the private key of `BALANCER_ATTACKER_ADDRESS` any transaction from it can't be included in blocks after the hard-fork
